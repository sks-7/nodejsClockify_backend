...
import { IUserBody } from "../types/LoginUserBody.js";
...
export const loginUser = async (
  ...
  try {
    ...
    const token = await GeneratingToken(
      find_user._id,
      find_user.email,
      "sachinrockstar",
      "7 days"
...
export const forgotPassword = async (
  ...
  try {
    ...
    const forgotToken = GeneratingToken(
      oldUser._id,
      oldUser.email,
      secret,
      "5m"
...
export const resetPasswordVerifyuser = async (req: Request, res: Response) => {
  ...
  try {
    const verify: any = await verifyToken(forgotToken, secret);
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    const verify: any = verifyToken(forgotToken, secret);
